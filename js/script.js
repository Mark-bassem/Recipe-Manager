﻿(() => {
  ...
  // يرجع البريد الإلكتروني للمستخدم المسجّل دخوله الحالي.
  function getCurrentUserEmail() {
    const currentUser = getCurrentUser();
    return normalizeEmail(currentUser?.email || "");
  }

  // يحافظ على حالة رؤوس التنقل حسب تسجيل الدخول.
  ...
  function requireAuth() {
    const protectedPages = new Set(["add-recipe.html"]);
  ...
  function loadProfile() {
    ...
    const currentUserEmail = normalizeEmail(profileUser.email || "");
    const userRecipes = state.recipes.filter(
      (recipe) => normalizeEmail(recipe.createdBy || "") === currentUserEmail,
    );
    const safeRecipesCount = escapeHtml(String(userRecipes.length));
    const userRecipesMarkup =
      userRecipes.length === 0
        ? `<p>${t("You have not added any recipes yet.")}</p>`
        : `<ul class="profile-recipes">
            ${userRecipes
              .map(
                (recipe) =>
                  `<li><button class="link-button" onclick="RecipeManager.goToRecipe('${encodeURIComponent(
                    recipe.name,
                  )}')">${escapeHtml(
                    getRecipePresentation(recipe).name,
                  )}</button></li>`,
              )
              .join("")}
          </ul>`;
      ...
      <article class="card profile-card">
        <div class="card-body">
          ...
          <div class="profile-stats">
            ...
            <p><strong>${t("Added Recipes:")}</strong> ${safeRecipesCount}</p>
          </div>
        </div>
      </article>
      <article class="card profile-card">
  ...
  function addRecipe() {
    const currentUser = getCurrentUser();
    if (!currentUser) {
      const redirect = encodeURIComponent("add-recipe.html");
      window.location.href = `login.html?redirect=${redirect}`;
      return;
    }
    ...
    state.recipes.push({
      ...
      instructions,
      cuisine,
      createdBy: normalizeEmail(currentUser.email || ""),
    });
    saveState();
